    ...
    while True:
        try:
            ...
            if res.status_code == 200:
                ...
                print(f"    -> API Hatası (HTTP {res.status_code}), 5 saniye sonra tekrar deneniyor... (Deneme: {attempt})")
        except Exception as e:
            print(f"    -> İstek Hatası ({e}), 5 saniye sonra tekrar deneniyor... (Deneme: {attempt})")
    ...
    for dist in dists:
        ...
        for prof in profiles:
            ...
            for meter_type in [1, 3]:
                ...
                while True:
                    try:
                        # Timeout ekleyerek sonsuza kadar beklemesini engelliyoruz
                        res = requests.post(DATA_URL, json=req_body, headers=headers, timeout=60)
                        if res.status_code == 200:
                            resp_json = res.json()
                            ...
                            print(f"    -> Hata: HTTP {res.status_code} - {res.text}. 5 saniye sonra tekrar deneniyor... (Deneme: {attempt})")
                    except Exception as e:
                        print(f"    -> İstek Hatası (Timeout/Bağlantı): {e}. 5 saniye sonra tekrar deneniyor... (Deneme: {attempt})")
